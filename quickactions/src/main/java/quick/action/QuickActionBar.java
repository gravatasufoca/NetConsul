...
public class QuickActionBar extends NewPopUpWindow {
    ...
    public QuickActionBar(View anchor) {
        ...
        loadAnimation = AnimationUtils.loadAnimation(anchor.getContext(),
                R.anim.translation);
    ...
    public void show() {
        ...
        displayDirectionChoice(
                ((popupOnTop) ? R.id.arrow_down : R.id.arrow_up),
                anchorRect.centerX());
        ...
        if (startAnimation)
            itemArray.startAnimation(loadAnimation);
    }

    private void setAnimationStyle(boolean popupOnTop) {
        switch (animationStyle) {
            case GROW_FROM_LEFT:
                window.setAnimationStyle((popupOnTop) ? R.style.Animations_PopUpMenu
                        : R.style.Animations_PopDownMenu);
                break;

            case GROW_FROM_CENTER:
                window.setAnimationStyle((popupOnTop) ? R.style.Animations_GrowFromTop
    ...
    private void displayDirectionChoice(int directionChoice, int requestedX) {
        final View showArrow = (directionChoice == R.id.arrow_up) ? arrowUp
                : arrowDown;
        final View hideArrow = (directionChoice == R.id.arrow_up) ? arrowDown
                : arrowUp;
    ...
    private void setUpList() {
        ...
        for (int i = 0; i < itemList.size(); i++) {
            ...
            LinearLayout actionHolder = (LinearLayout) inflater.inflate(
                    R.layout.quickactions, null);
            ImageView actionImage = (ImageView) actionHolder
                    .findViewById(R.id.image);
            TextView actiontxt = (TextView) actionHolder
